...
url1 = "https://raw.githubusercontent.com/andersonzambeli/ine5649/refs/heads/main/tabelas/SINAN/tab_SINAN__florianopolis.csv"
url2 = "https://raw.githubusercontent.com/andersonzambeli/ine5649/refs/heads/main/tabelas/precipitacao/precipitacao.csv"
# Criando a tabela de dados meteorologicos acumlados por dia
dados_meteorologicos <- read.csv(url2)
...
dados_notificacoes <- read.csv(url1)
...
dados_1 = df_mensal[df_mensal$ano < 2020,]
dados_2 = df_mensal
...
treino4 = treino1
teste4 = teste1
treino4$contagem = (treino4$contagem)
ts4_treino= ts(log(treino4$contagem + 1), frequency = 12)
plot(ts4_treino)
teste4$contagem = (teste4$contagem)
ts4_test = ts(log(teste4$contagem + 1), start=end(ts4_treino), frequency = 12)
plot(ts4_test)

treino5 = treino2
teste5 = teste2
treino5$contagem = (treino5$contagem)
ts5_treino= ts(log(treino5$contagem + 1), frequency = 12)
plot(ts5_treino)
teste5$contagem = (teste5$contagem)
ts5_test = ts(log(teste5$contagem + 1), start=end(ts5_treino), frequency = 12)
plot(ts5_test)

treino6 = treino3
teste6 = teste3
treino6$contagem = (treino6$contagem)
ts6_treino= ts(log(treino6$contagem + 1), frequency = 12)
plot(ts6_treino)
teste6$contagem = (teste6$contagem)
ts6_test = ts(log(teste6$contagem + 1), start=end(ts6_treino), frequency = 12)
plot(ts6_test)
...
# Usando os modelos arima para os dados4 que é intervalo entre 2010 e 2019, onde a contagem de notificações foi trasnformada
# usando log(x + 1)

# Usando arima com Seasonal = false
modelo_arima = auto.arima(ts4_treino, seasonal = FALSE)
checkresiduals((modelo_arima))
summary(modelo_arima)
prev_arima <- forecast(modelo_arima, h = 24)
teste = cbind(teste4,prev_arima$mean)


teste$date <- as.Date(teste$Data)
ggplot(teste, aes(x=Data, y=log(contagem+1))) +
  geom_line() +
  geom_line(aes(y = prev_arima$mean, color = "ARIMA"),
            linetype = "dashed") +
  theme(legend.position = "bottom")
...
# Usando arima com Seasonal = True
modelo_sarima = auto.arima(ts4_treino, seasonal = TRUE, D = 1   )
summary(modelo_sarima)
checkresiduals((modelo_sarima))
prev_sarima <- forecast(modelo_sarima, h=24)
teste = cbind(teste4,prev_sarima$mean)
...
# Usando os modelos arima para os dados5 que é intervalo entre 2010 e 2024, onde a contagem de notificações foi trasnformada
# usando usando log(x + 1)

# Usando arima com Seasonal = false
modelo_arima = auto.arima(ts5_treino, seasonal = FALSE)
checkresiduals((modelo_arima))
summary(modelo_arima)
prev_arima <- forecast(modelo_arima, h = 24)
teste = cbind(teste5,prev_arima$mean)
...
# Usando arima com Seasonal = True
modelo_sarima = auto.arima(ts5_treino, seasonal = TRUE, D = 1   )
summary(modelo_sarima)
checkresiduals((modelo_sarima))
prev_sarima <- forecast(modelo_sarima, h=24)
teste = cbind(teste5,prev_sarima$mean)
...
# Usando os modelos arima para os dados6 que é intervalo entre 2020 e 2024, onde a contagem de notificações foi trasnformada
# usando log(x + 1)

# Usando arima com Seasonal = false
modelo_arima = auto.arima(ts6_treino, seasonal = FALSE)
checkresiduals((modelo_arima))
summary(modelo_arima)
prev_arima <- forecast(modelo_arima, h = 24)
teste = cbind(teste6,prev_arima$mean)
...
################################### MELHOR ###########################################
# Usando arima com Seasonal = True
modelo_sarima = auto.arima(ts6_treino, seasonal = TRUE, D = 1   )
summary(modelo_sarima)
checkresiduals((modelo_sarima))
prev_sarima <- forecast(modelo_sarima, h=24)
teste = cbind(teste6,prev_sarima$mean)
...
# Usando suavização exponencial em dados4

SES = ses(ts4_treino, h=24)
summary(SES)
HOLT = holt(ts4_treino, h=24)
summary(HOLT)
HW_ad = hw(ts4_treino, seasonal = "additive", h=24)
summary(HW_ad)

SES.predito = forecast(SES,h = 24)
HOLT.predito = forecast(HOLT,h = 24)
HWad.predito = forecast(HW_ad,h = 24)
teste = cbind(teste4,SES.predito$mean,HOLT.predito$mean,HWad.predito$mean)
...
# Usando suavização exponencial em dados5

SES = ses(ts5_treino, h=24)
summary(SES)
HOLT = holt(ts5_treino, h=24)
summary(HOLT)
HW_ad = hw(ts5_treino, seasonal = "additive", h=24)
summary(HW_ad)

SES.predito = forecast(SES,h = 24)
HOLT.predito = forecast(HOLT,h = 24)
HWad.predito = forecast(HW_ad,h = 24)
teste = cbind(teste5,SES.predito$mean,HOLT.predito$mean,HWad.predito$mean)
...
# Usando suavização exponencial em dados6

SES = ses(ts6_treino, h=24)
summary(SES)
HOLT = holt(ts6_treino, h=24)
summary(HOLT)
HW_ad = hw(ts6_treino, seasonal = "additive", h=24)
summary(HW_ad)

SES.predito = forecast(SES,h = 24)
HOLT.predito = forecast(HOLT,h = 24)
HWad.predito = forecast(HW_ad,h = 24)
teste = cbind(teste6,SES.predito$mean,HOLT.predito$mean,HWad.predito$mean)
